...
double det(vector<vector<double>>& a) {
	...
	rep2(i,0,n) {
		int b = i;
		rep2(j,i+1,n) if (fabs(a[j][i]) > fabs(a[b][i])) b = j;
		if (i != b) swap(a[i], a[b]), res *= -1;
		res *= a[i][i];
		if (res == 0) return 0;
		rep2(j,i+1,n) {
			double v = a[j][i] / a[i][i];
			if (v != 0) rep2(k,i+1,n) a[j][k] -= v * a[i][k];
		}
	}
	return res;
}
...
int main() {
	rep2(n,0,4) {
		...
		rec(0,0,mat,[&]() {
			rep2(i,0,n) rep2(j,0,n) mat3[i][j] = mat[i][j];
