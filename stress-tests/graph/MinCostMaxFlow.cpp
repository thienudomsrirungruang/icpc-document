...
ll MinCostMatching(const vector<vd>& cost, vi& L, vi& R) {
	...
	rep2(i,0,n) {
		u[i] = cost[i][0];
		rep2(j,1,n) u[i] = min(u[i], cost[i][j]);
	}
	rep2(j,0,n) {
		v[j] = cost[0][j] - u[0];
		rep2(i,1,n) v[j] = min(v[j], cost[i][j] - u[i]);
	}
...
void testMatching() {
	rep2(it,0,100000) {
		...
		rep2(i,0,N) rep2(j,0,M) co[i][j] = (rand() % 10) + 2;
		vi L, R;
		ll v = MinCostMatching(co, L, R);
		int S = N+M, T = N+M+1;
		MCMF mcmf(N+M+2);
		rep2(i,0,N) mcmf.addEdge(S, i, 1, 0);
		rep2(i,0,M) mcmf.addEdge(N+i, T, 1, 0);
		rep2(i,0,N) rep2(j,0,M) mcmf.addEdge(i, N+j, 1, co[i][j] - 2);
		mcmf.setpi(S);
		auto pa = mcmf.maxflow(S, T);
...
void testNeg() {
	...
	rep2(it,0,1000000) {
		...
		MCMF mcmf(N);
		MCMF2 mcmf2(N);
		rep2(i,0,N) rep2(j,0,N) ed[i][j] = 0;
		rep2(eid,0,M) {
