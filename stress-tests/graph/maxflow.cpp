...
int main() {
	rep2(it,0,500000) {
		...
		vector<ll> flows(n);
		rep2(i,0,n) for(auto &e: pr.g[i]) if (e.f > 0) {
			assert(e.c >= 0);
			flows[i] += e.f;
			flows[e.dest] -= e.f;
		}
		assert(flow == flows[s]);
		assert(flow == -flows[t]);
		rep2(i,0,n) if (i != s && i != t) assert(flows[i] == 0);

		// Conservation of flow for Dinic
		vector<ll> dinicFlows(n);
		rep2(i,0,n) for(auto &e: dinic.adj[i]) {
			assert(e.flow() <= e.oc);
			dinicFlows[i] += e.flow();
		...
		vector<ll> ekFlows(n);
		rep2(i,0,n) for(auto &e: origEk[i]) {
			int nc = ek[i][e.first];
			assert(nc >= 0);
