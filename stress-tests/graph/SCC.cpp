...
template<class G> vi scc(G &g) {
	...
	comp.assign(n, -1);
	rep2(i,0,n) if (comp[i] == -1) dfs(i, g);
	return comp;
}
}
...
int main() {
	...
	for (int N = 0; N <= 4; N++) {
		...
		rep2(bits,0,(1 << (N*N))) {
			...
			rep2(i,0,N) assert(comp[i] >= 0 && comp[i] < ncomps);
			rep2(i,0,N) for(auto &j: adj[i]) assert(comp[j] <= comp[i]);
			rep2(i,0,N) {
				seen.assign(N, 0); seen[i] = 1;
				rep2(it,0,N) {
					rep2(j,0,N) if (seen[j]) for(auto &k: adj[j]) seen[k] = 1;
				}
				rep2(j,0,N) {
